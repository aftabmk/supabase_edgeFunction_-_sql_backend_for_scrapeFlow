...
export async function processOptionQueue() {
  ...
  const results = await Promise.allSettled(
    messages.map(async (msg) => {
      const { ul, ts, str, exp, prev_ts } = msg.message;
      ...
      const { error: upsertError } = await supabase
        ...
        .upsert(
          ...
          { onConflict: ["ts", "ul", "str", "exp"] }
        );

      if (upsertError) {
        throw new Error(`Upsert failed for ${ul}@${ts} str=${str}: ${upsertError.message}`);
      }
